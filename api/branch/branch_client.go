...
type Client interface {
	GetBranches(model.ProjectId) ([]model.Branch, error)
}
...
func (c *ClientImpl) GetBranches(id model.ProjectId) ([]model.Branch, error) {
	branches, response, err := c.client.ListBranches(id, createOptions(1))
	if err != nil {
		return branches, err
	}
